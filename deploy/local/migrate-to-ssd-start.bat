...
echo ================================
echo  biaoqian - local migrate to SSD start
echo ================================
...
pushd "%OLD_ROOT%"
docker compose -p biaoqian_local -f docker-compose.yml -f docker-compose.dev.yml down
if errorlevel 1 (
...
if not exist "%NEW_ROOT%\deploy\local\env.bat" (
    echo [ERROR] New root local env file not found: %NEW_ROOT%\deploy\local\env.bat
    pause
    exit /b 1
)
...
pushd "%NEW_ROOT%"
call "%NEW_ROOT%\deploy\local\env.bat"
...
docker inspect biaoqian_local-postgres-1 --format "{{.State.Health.Status}}" 2>nul | findstr /I "healthy" >nul
if errorlevel 1 (
    set /a RETRIES+=1
    timeout /t 5 /nobreak >nul
    goto deps_loop
)
docker inspect biaoqian_local-elasticsearch-1 --format "{{.State.Health.Status}}" 2>nul | findstr /I "healthy" >nul
if errorlevel 1 (
    set /a RETRIES+=1
    timeout /t 5 /nobreak >nul
    goto deps_loop
