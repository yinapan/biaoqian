...
| 特效 | GIF 动图 | 现有 GIF 文件（`特效/` 目录） | 卡片显示首帧 PNG 缩略图，悬停替换 src 加载 GIF |
...
- 特效 GIF：`static/previews/2/{resource_id}.gif`，缩略图：`static/previews/2/{resource_id}_thumb.png`
...
gif.seek(0)  # 首帧
gif.save(thumb_path, "PNG")
```
- 单个 GIF 3-6MB，20 张卡片直接加载 = 60-120MB，内网也扛不住
- 导入时用 Pillow 提取首帧生成 PNG 缩略图（< 50KB）
- 搜索结果卡片默认显示 PNG 缩略图
- 鼠标悬停时通过替换 `<img src>` 加载完整 GIF（非 CSS 控制播放）
- 详情弹窗直接加载完整 GIF
